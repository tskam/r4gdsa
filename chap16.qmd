...
```{r echo=TRUE, eval=TRUE}
mpsz <- st_read(dsn = "chap16/data/geospatial", layer = "MP14_SUBZONE_NO_SEA_PL")
hexagons <- st_read(dsn = "chap16/data/geospatial", layer = "hexagons") 
eldercare <- st_read(dsn = "chap16/data/geospatial", layer = "ELDERCARE") 
```
...
```{r message=FALSE}
ODMatrix <- read_csv("chap16/data/aspatial/OD_Matrix.csv", skip = 0)
```
...
![](chap16/img/image3.jpg){width=65%}
...
![](chap16/img/image4.jpg){width=25%}
