...
```{r}
mdata <- read_rds("chap14/data/rawdata/mdata.rds")
```
...
In the code code chunk below, [`st_jitter()`](https://r-spatial.github.io/sf/reference/st_jitter.html) of **sf** package is used to move the point features by 1m to avoid overlapping point features.
...
#| eval: false
set.seed(1234)
resale_split <- initial_split(mdata_jittered, 
                              prop = 7.5/10,)
...
The result shows that 48 neighbour points will be the optimal bandwidth to be used if adaptive bandwidth is used for this data set.
