...
saveGIF({
  for (t in 10:352) {
    plot(kde_yday, tselect = t,
         main = paste("KDE - Day", t))
  }
}, movie.name = "chap06/kde_animation.gif", 
interval = 0.2)
```
...
#| eval: false
saveGIF({
  for(i in 1:length(kde_yday$time)) {
    plot(kde_yday, i)
  }
}, movie.name = "kde_animation.gif", interval = 0.25)
```
...
Next, we will create study area boundary by using *kbb_sf* data by using the code chunk below.
